...
void setup(uint32_t APPLICATION_ID, char* INSTALL_PATH) {
	auto result = discord::Core::Create(APPLICATION_ID, DiscordCreateFlags_NoRequireDiscord, &core);
	...
	state.core->ActivityManager().RegisterCommand(INSTALL_PATH);
...
extern "C" {
	...
	__declspec(dllexport) void DLLMain(uint32_t APPLICATION_ID, char* INSTALL_PATH) {
		setup(APPLICATION_ID, INSTALL_PATH);
